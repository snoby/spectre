...
namespace SpectreX
{
  ...
  public:
    ...
    uint64_t next()
    {
     ...
     uint64_t sum = s[0] + s[3]; // Calculate the sum of s[0] and s[3]
     uint64_t leftRotatedSum = (sum << 23) | (sum >> (64 - 23)); // Perform left rotation with wraparound
     uint64_t res = leftRotatedSum + s[0]; // Add s[0] to the left rotated sum
